...
const getAvailableBalance = async (req, res) => {
  try {
   ...
   const todayCommission = await Income.sum('comm', {
    ...
    const teamDesposit = notes.filter(u => u.active_status === 'Active').reduce((sum, user) => sum + (user.package || 0), 0);
    
            // Fetch total withdraw of team members
        const totalWithdrawData = await Withdraw.findAll({
          where: {
            user_id: ids.length ? { [Op.in]: ids } : null,
            status: 'Approved' // Optional: only count approved withdrawals
          },
          attributes: ['amount']
        });
        
        const totalteamWithdraw = totalWithdrawData.reduce((sum, record) => sum + (record.amount || 0), 0);
        
            
    res.json({ available_balance: balance,withdraw:totalWithdraw,totlinvest:totalInvestment,todayReward,totalReward,todayCommission,totalCommission,totalReferral,totalMember,totalValid,teamDesposit,totalteamWithdraw });
  } catch (error) {
