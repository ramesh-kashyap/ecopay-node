...
const Investment = require("../models/Investment"); // User Model Import Karein
...
const withdrawRequest = async (req, res) => {
    try {
        ...
        const checkIdfree = await Investment.findOne({ where: { user_id: user.id, status: 'Active',active_from: 'Admin' } });
        ...
        if (checkIdfree) {
            ...
            if(userDirect<10)
            {
               return res.status(400).json({ error: 'You Need 10 Direct Users for withdrawals' });  
            }
