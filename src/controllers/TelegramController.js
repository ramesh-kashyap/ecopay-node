...
const startTrade = async (req, res) => {
    try {
        ...
        // check user not claimed 
        
        if(userDetail.total_reward > 0)
        {
                ...
                if (!incomeExists) {
                  const commission = userDetail.total_reward;
                  logger.info(`Crediting commission ${commission} to user ${userDetail.username}`);
                  ...
                  logger.info(`Level income added for user ID: ${userDetail.id}`);
                } else {
                  logger.info(`Reward already claimed today by user ID: ${userDetail.id}`);
                }
...
const getLastTrade = async (req, res) => {
    try {
        ...
        const totalReward = user.total_reward || 0;
        ...
        const lastUpdated =  user.lastUpdated;
            // Example Usage:
        return res.json({ success: true, lastTrade, todayroi, timeLeft,lastTradeTime,totalReward  });
...
const claimReward = async (req, res) => {
  const { telegram_id } = req.body;
  logger.info(`Received claimReward request for telegram_id: ${telegram_id}`);

  try {
    const user = await TelegramUser.findOne({ where: { telegram_id } });
    if (!user) {
      logger.warn(`Telegram user not found: ${telegram_id}`);
      return res.status(404).json({ success: false, message: "User not found" });
    }

    const userDetail = await User.findOne({ where: { telegram_id: user.id } });
    if (!userDetail) {
      logger.warn(`User detail not found for telegram ID: ${user.id}`);
      return res.status(404).json({ success: false, message: "User not found" });
    }
    
     const userBalance = await getBalance(userDetail.id);
   if (userBalance<100) 
    {
    return res.status(404).json({ success: false, message: "Insufficient Balance" });
    }
    ...
    await User.update(
      { todayroi: 0, total_reward: 0, lastTrade: null },
      { where: { id: userDetail.id } }
    );

    logger.info(`Reward claimed and data reset for user ID: ${userDetail.id}`);
    res.json({ success: true, message: "Reward claimed!" });
  } catch (error) {
    logger.error(`Error claiming reward for telegram_id ${telegram_id}: ${error.stack}`);
  ...
  logger.info(`Starting addLevelIncome for user ID: ${id}, Amount: ${amt}`);
  ...
  while (user_mid && user_mid !== 1 && cnt <= 5) {
    logger.info(`Level ${cnt} - Fetching sponsor for user ID: ${user_mid}`);
    ...
    if (!sponsorUser) {
      logger.warn(`Sponsor user not found at level ${cnt}, user_mid: ${user_mid}`);
      break;
    }
    ...
    logger.info(`Sponsor ID: ${sponsorId}, Status: ${sp_status}, VIP Level: ${vipLevel}`);
    ...
    if (sp_status === "Active" && vipLevel >= 1) {
      pp = amount * currentMultipliers[cnt - 1];
    }
    ...
    if (spid > 0 && cnt <= 5 && pp > 0) {
      logger.info(`Crediting ${pp} to sponsor ID ${spid} at level ${cnt}`);
      ...
      logger.debug(`No payout at level ${cnt} for sponsor ID ${spid}`);
    }
  ...
  logger.info(`addLevelIncome completed for user ID: ${id}`);
  return true;
};
